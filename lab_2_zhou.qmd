...
#variable name
f_1 <- mass~bill_l+bill_d+flip_l+species+sex+island
f_2 <- mass~bill_l+bill_d+flip_l+species+sex
f_3 <- mass~bill_d+flip_l+species+sex

mdl1<-lm(f_1, data=penguins_clean)
mdl2<-lm(f_2, data=penguins_clean)
mdl3<-lm(f_3, data=penguins_clean)
...
for loop 
  ...
  make preditcion on teh test set with fitted training model 
close loop: 
...
#Describe in words the replace argument.
  ...
  # size in sample larger than size in ex
```
...
```{r}
training_lm1 <- lm(f_1, data=train_df)
training_lm2 <- lm(f_2, data=train_df)
training_lm3 <- lm(f_3, data=train_df)
...
```{r}
calc_rmse(predict_test$mass, predict_test$model1)
```
...
rmse_predict_test<-predict_test |> 
  summarize(rmse_md1=calc_rmse(mass, model1),
            rmse_md2=calc_rmse(mass, model2),
            rmse_md3=calc_rmse(mass, model3)) 
rmse_predict_test
```
...
rmse_vec<-vector(length=folds)  # for storing results out of for loop
...
for( i in 1:folds){
  ...
  # Run for one model
  kfold_training_lm1 <- lm(f_1, data=kfold_train_df)
...
kfold_cv<-function(i,df,formula){
  ...
  kfold_rmse<-kfold_pred_df |> 
    summarize(rmse_md=calc_rmse(mass, md)) 
...
```{r}
# use function
kfold_cv(df=penguins_fold, formula=f_1)
```
...
```{r}
rmse_df<-data.frame(j=1:folds) |> mutate(rmse_mdl1=map_dbl(j, kfold_cv, df=penguins_fold,formula=f_1),
                                         rmse_mdl2=map_dbl(j,kfold_cv,df=penguins_fold,formula=f_2),
                                         rmse_mdl3=map_dbl(j,kfold_cv,df=penguins_fold,formula=f_3))
